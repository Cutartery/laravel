...
<form id="qwe" action="#" method="post" >
    @csrf
    <div class=" page-content clearfix">
        <div id="products_style">
            <div class="border clearfix">
                <span class="l_f">
                <a href="{{route('product_category_add')}}" title="添加分类" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加分类</a>
                </span>
            </div>
                <div class="table_menu_list" id="testIframe">
                    <table class="table table-striped table-bordered table-hover" id="sample-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td width="10%">{{$v->id}}</td>
                                <td style="text-align:left" width="20%">{{str_repeat('-', 8*(count(explode('-', $v->ify_path))-2)) . $v->ify_name}}</td>
                                <td width="20%">{{$v->ify_pid}}</td>
                                <td width="20%">{{$v->ify_path}}</td>
                                <td class="td-status"><span class="label label-success radius">已启用</span></td>
                                <td class="td-manage">
                                    <a onClick="member_stop(this,'10001')" href="javascript:;" title="停用" class="btn btn-xs btn-success">
                                        <i class="icon-ok bigger-120"></i>
                                    </a>
                                    <a title="编辑" onclick="member_edit('编辑','/product_category_insert/','{{$v->id}}','','510')" href="javascript:;"
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            layer.msg('已删除!', { icon: 1, time: 1000 });
                $.ajax({
                    type:'POST',
                    url:'{{route("doproduct_category_index")}}',
                    data:{id:id,_token:"{{csrf_token()}}"},
                    success:function(data){
                        window.location.href="{{route('product_category_index')}}"
                    }
                })
            });
